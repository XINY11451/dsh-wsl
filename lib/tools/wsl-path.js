...
export function createWslPathTool({ config, runner }) {
  return {
    ...
    // `ToolCallView` knows exactly three cards: generic, terminal, diff. A card
    // outside that set has no renderer, and a generic card has no `description`
    // slot — both of those were wrong here until the platform type was read.
    presentCall: (args) => ({ card: 'generic', title: `Convert path ${args.path}` }),
  }
}
