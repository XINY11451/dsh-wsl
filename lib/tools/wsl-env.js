...
export function createWslEnvTool({ config, runner }) {
  return {
    ...
    // See wsl-path: only generic/terminal/diff exist, and `description` is a
    // terminal-card field rather than a generic one.
    presentCall: () => ({ card: 'generic', title: 'Inspect the WSL environment' }),
  }
}
