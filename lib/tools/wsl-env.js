...
export function createWslEnvTool({ config, runner }) {
  return {
    ...
    async execute(args) {
      ...
      // Where the session's own files live is the one fact here that changes
      // what the caller should choose to DO, so it is stated rather than implied.
      const workspace = workspaceLine(process.cwd())
